...
func main() {
	...
	for _, trailHead := range trailHeads {
		total += findAccessibleSummits(theMap, trailHead)
	}

	fmt.Println(total)
}

func findAccessibleSummits(theMap [][]int, position cord) (total int) {
	...
	if position.x > 0 && theMap[position.y][position.x-1] == theMap[position.y][position.x]+1 {
		total += findAccessibleSummits(theMap, cord{position.x - 1, position.y})
	}
	// right
	...
	if position.y > 0 && theMap[position.y-1][position.x] == theMap[position.y][position.x]+1 {
		total += findAccessibleSummits(theMap, cord{position.x, position.y - 1})
	}
	// down
